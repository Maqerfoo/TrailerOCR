...
def test(net, images_folder, output_folder, scaled_height):
    ...
    for image_name in pbar:
        ...
        with open('{}'.format(os.path.join(output_folder, '{}.txt'.format(prefix))), 'w') as f:
            reshaped_pred_polys = []
            for id in range(polys.shape[0]):
                ...
                reshaped_pred_polys.append(np.array([int(polys[id, 0] / scale_x), int(polys[id, 1] / scale_y), int(polys[id, 2] / scale_x), int(polys[id, 3] / scale_y), int(polys[id, 4] / scale_x), int(polys[id, 5] / scale_y), int(polys[id, 6] / scale_x), int(polys[id, 7] / scale_y)]).reshape((4, 2)))
